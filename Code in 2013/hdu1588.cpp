...
题意：给你k , b , n , mod ,求斐波那契项第 b项 到 第 ki+b 项的和，i<n ，i >=0
就是 F(b)+F(k+b)+F(2k+b)+....+F((n-1)k+b) 对于斐波那契，我们还是用快速幂做，所以化简成矩阵的形式就是 B^b+ B^（k+b）+B^(2k+b) +...+B^((n-1)k+b)
等于 B^b *（B^k+ B^2k+ B^3k+ ... + B^(n-1)k）然后括号里面的用二分解
*/
#include <cstdio>
#include <iostream>
#include <algorithm>
